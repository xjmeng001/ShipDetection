...
def fit_one_epoch(net, criterion, epoch, epoch_size, gen, Epoch, anchors, cfg, cuda):
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            #   计算损失
            # ----------------------#
            r_loss, c_loss = criterion(out, anchors, targets)
            loss = cfg['loc_weight'] * r_loss + c_loss
    ...
    torch.save(model.state_dict(),
               'logs/Epoch%d-Total_Loss%.4f-Conf_Loss.pth%.4f-Regression_Loss%.4f.pth' % (
               (epoch + 1), (total_c_loss + total_r_loss) / (epoch_size + 1),
               total_c_loss / (iteration + 1), total_r_loss / (iteration + 1)))
    return
...
if __name__ == "__main__":
    ...
    #   获得训练用的人脸标签与坐标
    # --------------------------------#
    training_dataset_path = './data/label.txt'
    # -------------------------------#
    #   主干特征提取网络的选择
    #   mobilenet或者resnet50
    # -------------------------------#
