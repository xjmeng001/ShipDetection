...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    train_loss.append(total_loss/(epoch_size+1))
    eval_loss.append(val_loss/(epoch_size_val+1))
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------#
    anchors_path = 'model_data/ship_anchors.txt'
    ...
    f = open("logs/loss.txt", 'w')
    train_loss = list(map(str, train_loss))
    eval_loss = list(map(str, eval_loss))
    print(train_loss)
    f.write(",".join(train_loss))
    f.write('\n')
    f.write(",".join(eval_loss))
    f.close()
