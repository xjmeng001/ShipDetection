...
with open(results_files_path + "/results.txt", 'w') as results_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        if draw_plot:
            ...
            plt.plot(rec, prec, '-o', markersize=2)
            ...
            # set plot title
            plt.title(text, fontdict={'size': 14})
            ...
            plt.plot(score, rec, "-o", color='g', markersize=1, label='Recall')
            plt.plot(score, prec, "-s", color='m', markersize=1, label='Precision')
            plt.plot(score, F1, "-o", color='orangered', markersize=1, label='F1')
            plt.title(Precision_text + ', ' + Recall_text + "\nscore_threhold=0.5", fontdict={'size': 14})
            plt.xlabel('Score_Threhold', fontdict={'size': 14})
            plt.ylabel('P-R-F1', fontdict={'size': 14})
            axes = plt.gca()  # gca - get current axes
            axes.set_xlim([0.0, 1.0])
            axes.set_ylim([0.0, 1.05])  # .05 to give some extra space
            plt.xticks(size=12)
            ...
            fig.savefig(results_files_path + "/arf.png")
